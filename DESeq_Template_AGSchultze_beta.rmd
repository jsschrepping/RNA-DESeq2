...
We define functions used for analysis and visualization of our data. All of our plots are based on ggplot2, so you can add different themes our change scales, etc. 
E.g.: DEcompare$KEGGplot+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
  
  to make the axis texts vertically. 
...
for(i in 2:length(names(DEresults))){
  ...
  sheet <- createSheet(ExcelOutput, sheetName = paste(substr(names(DEresults[i]),1 , 22),"_upDEGs"))
  addDataFrame(DEresults[[i]]@DE_genes$up_regulated_Genes, sheet, startRow=1, startColumn=1, row.names = FALSE)
  sheet <- createSheet(ExcelOutput, sheetName = paste(substr(names(DEresults[i]),1 , 20),"_downDEGs"))
